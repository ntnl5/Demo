#!/usr/bin/env node
// Dependency-free rebrand-kit builder. Node 18+, no npm install, ever. See docs/REBRAND-KIT.md.
//
// Usage:
...
// The footer's visible opening hours used to be hardcoded prose while schema.org read
// business.hours, so the two could disagree and nothing would catch it. Both now come
// from the same 24h values: "08:00" renders as "8am", "14:30" as "2.30pm".
...
// Optional legal/privacy details render either as the client's real value or as the
// bracketed .ph-flag placeholder that docs/ONBOARDING.md tells you to grep for. Doing it here
// rather than as an {{#if}}/{{#unless}} pair around every field keeps the legal pages
// readable, and means "unset" is decided in exactly one place.
//
// The result is markup, so templates must read these with {{{triple}}} braces. The value
// is escaped here, so a business name containing & or " is still safe.
function flagged(value, placeholderLabel) {
  if (isUnset(value)) return `<span class="ph-flag">[${placeholderLabel}]</span>`;
...
// Output minification
// ---------------------------------------------------------------------------
// Source stays readable and commented; the browser gets the compact version. This
// matters beyond transfer size, because style and script parsing cost scales with
// *uncompressed* bytes, and that parse is on the critical path to first paint.
//
// Deliberately conservative. Nothing is renamed or reordered, only whitespace and
// comments are removed, and the rendered output still has to clear gates 10 and 11,
// which reparse every inline <script> and ld+json block. A minifier that broke the
// JavaScript would fail the build rather than ship.

// Splits on string and regex literals so comment stripping never touches their contents.
function stripJsComments(js) {
  ...
  while (i < js.length) {
    ...
    if (ch === '"' || ch === "'" || ch === '`') {
      const quote = ch;
      let j = i + 1;
      while (j < js.length && js[j] !== quote) j += js[j] === '\\' ? 2 : 1;
      out += js.slice(i, j + 1);
      i = j + 1;
      continue;
    }
    ...
    if (ch === '/' && js[i + 1] !== '/' && js[i + 1] !== '*') {
      ...
      if (prev === '' || '(,=:[!&|?{};+-*%~^<>'.includes(prev)) {
        ...
        if (j > 0) { out += js.slice(i, j + 1); i = j + 1; continue; }
      }
    }
    if (ch === '/' && js[i + 1] === '/') { while (i < js.length && js[i] !== '\n') i++; continue; }
    if (ch === '/' && js[i + 1] === '*') { const e = js.indexOf('*/', i + 2); i = e === -1 ? js.length : e + 2; continue; }
    out += ch;
    i++;
  }
  return out;
...
// HTML comments are for whoever edits the template, not for the browser. Conditional
// comments and the ld+json block are left alone.
function minifyHtml(html) {
  let out = html.replace(/<!--(?!\[if)[\s\S]*?-->/g, '');
  out = out.replace(/<style>([\s\S]*?)<\/style>/g, (m, css) => '<style>' + minifyCss(css) + '</style>');
  out = out.replace(/<script>([\s\S]*?)<\/script>/g, (m, js) => '<script>' + minifyJs(js) + '</script>');
  return out.replace(/\n{2,}/g, '\n');
}
...
function computeView(data, slug) {
  ...
  const hours = (view.business.hours = view.business.hours || {});
  hours.weekdayDisplay = `${toDisplayTime(hours.weekdayOpen)} to ${toDisplayTime(hours.weekdayClose)}`;
  hours.saturdayDisplay = `${toDisplayTime(hours.saturdayOpen)} to ${toDisplayTime(hours.saturdayClose)}`;
  ...
  //
  // Stars are rendered here rather than by the carousel script, which used to draw a
  // hardcoded five for every testimonial regardless of what the review actually said.
  // A four-star review has to show four stars. No rating means no stars at all, never a
  // default of five: an invented rating is the one thing this repo must not do.
  const starColour = (view.brand && view.brand.colors && view.brand.colors.star) || '#f2b134';
  const starSvg = `<svg viewBox="0 0 24 24" fill="${escapeHtml(starColour)}"><path d="M12 2l3 6.5 7 .8-5.2 4.7 1.4 6.9L12 17.6 5.4 20.9l1.4-6.9L1.6 9.3l7-.8L12 2Z"/></svg>`;
  for (const t of Array.isArray(view.testimonials) ? view.testimonials : []) {
    ...
    const hasRating = Number.isInteger(n) && n >= 1 && n <= 5;
    t.starsHtml = hasRating ? starSvg.repeat(n) : '';
    // The label beside the stars is the same fact written twice, and two copies of a fact
    // drift. It now derives from the rating unless it was set explicitly, which keeps the
    // override available for non-numeric labels like "Sample" on the demo.
    if (isUnset(t.ratingLabel) && hasRating) t.ratingLabel = n.toFixed(1);
  }
  // The inline SVG favicon is a data: URI, so its colours have to be URL-encoded
  // (`#c9772e` becomes `%23c9772e`). Deriving it means a rebrand only ever touches
  // brand.colors and the tab icon follows.
  view.brand = view.brand || {};
  view.brand.colors = view.brand.colors || {};
  const enc = (hex) => (typeof hex === 'string' ? hex.replace(/^#/, '%23') : hex);
  view.brand.colors.accentEnc = enc(view.brand.colors.accent);
  view.brand.colors.logoMarkInkEnc = enc(view.brand.colors.logoMarkInk);
  view.legal = view.legal || {};
  view.legal.year = view.legal.year || new Date().getFullYear();
  // Shared by the footer disclosure, terms.html and privacy.html, so the same company
  // details cannot say one thing on one page and something else on another.
  view.legal.registeredNameHtml = flagged(view.legal.registeredName, 'REGISTERED BUSINESS NAME');
  view.legal.jurisdictionHtml = flagged(view.legal.jurisdiction, 'SCOTLAND / ENGLAND &amp; WALES');
  view.legal.companyNumberHtml = flagged(view.legal.companyNumber, 'SC000000');
  view.legal.registeredOfficeHtml = flagged(view.legal.registeredOffice, 'REGISTERED OFFICE ADDRESS');
  view.legal.vatNumberHtml = flagged(view.legal.vatNumber, 'GB000000000');
  view.legal.postalAddressHtml = flagged(view.legal.postalAddress, 'POSTAL ADDRESS');
  view.legal.lastUpdatedHtml = flagged(view.legal.lastUpdated, 'DATE');
  view.business.phoneDisplayHtml = flagged(view.business.phoneDisplay, 'PHONE');
  view.business.streetAddressHtml = flagged(view.business.streetAddress, 'CLIENT ADDRESS');
  view.business.emailHtml = flagged(view.business.email, 'EMAIL');
  view.privacy = view.privacy || {};
  view.privacy.formProviderHtml = flagged(view.privacy.formProvider, 'FORM PROVIDER');
  view.privacy.dataLocationHtml = flagged(view.privacy.dataLocation, 'UK / EU / US');
  view.privacy.transferMechanismHtml = flagged(view.privacy.transferMechanism, 'TRANSFER MECHANISM');
  view.privacy.retentionPeriodHtml = flagged(view.privacy.retentionPeriod, 'RETENTION PERIOD');
  view.privacy.analyticsNoteHtml = flagged(
    view.privacy.analyticsNote,
    'IF ANALYTICS, ADVERTISING OR CALL TRACKING IS ADDED, A CONSENT BANNER IS REQUIRED. UPDATE THIS SECTION'
  );
  view.meta = view.meta || {};
...
// ---------------------------------------------------------------------------
// Validation gates. These run against the raw parsed JSON, before anything is rendered.
// ---------------------------------------------------------------------------
...
function runValidationGates(data, slug, liveFlag) {
  ...
  // canonical/og:image are derived from it, and a bad baseUrl would poison both.
  ...
  // Gate 2: every testimonial must record how it was verified, and any star rating it
  // shows must be a real whole-star value. A rating of 0, 6 or "five" would otherwise
  // render as no stars at all and nobody would notice the review had lost its rating.
  for (const t of data.testimonials || []) {
    ...
    if (!isUnset(t.rating) && !(Number.isInteger(t.rating) && t.rating >= 1 && t.rating <= 5)) {
      fail(
        `Gate 2 failed: testimonial "${label}…" has rating ${JSON.stringify(t.rating)}, which is not a whole number of stars from 1 to 5. Use the rating the customer actually left, or remove the field to show no stars.`
      );
    }
    // ratingLabel is normally derived from rating. It may be overridden with a word such
    // as "Sample", but an override that reads as a *number* and disagrees with the stars
    // beside it is the drift this derivation exists to prevent, so it fails the build.
    if (!isUnset(t.ratingLabel) && !isUnset(t.rating)) {
      const labelled = Number(String(t.ratingLabel).trim());
      if (Number.isFinite(labelled) && labelled !== Number(t.rating)) {
        fail(
          `Gate 2 failed: testimonial "${label}…" shows ${t.rating} stars but its ratingLabel reads "${t.ratingLabel}". Make them agree, or drop ratingLabel and let it derive from rating.`
        );
      }
    }
  }
  ...
  // Gates 5 and 6: meta.noindex and --live must agree.
  ...
  // Gate 14: the webfont request must ask for weight *ranges*, not a list of discrete
  // weights.
  //
  // Both families are variable, so one file covers the whole range. Asking for
  // "wght@400;500;600;700" makes Google emit a separate @font-face per weight, all
  // pointing at the same file, and the browser then downloads that file once per block.
  // On this build that was 31 blocks for 7 files and about 250KB of duplicate transfer.
  // It is invisible in the markup and only shows up in a network waterfall, which is
  // exactly the kind of regression a rebrand should not be able to introduce quietly.
  if (data.fonts && data.fonts.url) {
    const url = String(data.fonts.url);
    if (!/^https:\/\//.test(url)) {
      fail(`Gate 14 failed: fonts.url must be an absolute https:// URL. Got ${JSON.stringify(data.fonts.url)}.`);
    }
    for (const seg of url.match(/wght@[^&]*/g) || []) {
      if (seg.includes(';')) {
        fail(
          `Gate 14 failed: fonts.url asks for discrete weights in "${seg}". Use a range instead, e.g. "wght@400..700". ` +
            `These are variable fonts: a range is one file, while a semicolon-separated list makes the browser fetch the same file once per weight.`
        );
      }
    }
    if (!/display=swap/.test(url)) {
      fail(`Gate 14 failed: fonts.url is missing "display=swap". Without it the browser hides text until the webfont arrives, which delays First Contentful Paint.`);
    }
  }

  // Gate 13: a social icon must link somewhere real. An href of "#" or a relative path
  // gives no destination, fails WCAG 2.4.4, and looks like a broken site to a prospect.
  for (const soc of data.social || []) {
    const url = String(soc.url || '');
    if (!/^https:\/\/\S+$/.test(url)) {
      fail(
        `Gate 13 failed: social entry "${soc.label || '(unlabelled)'}" has url ${JSON.stringify(soc.url)}. Use the client's full https:// profile URL, or remove the entry. A social icon that links nowhere is worse than no icon.`
      );
    }
    if (!soc.label) {
      fail(`Gate 13 failed: social entry ${JSON.stringify(url)} has no label. The label is the icon's accessible name, so a screen reader can tell one from another.`);
    }
  }

  // Gate 8: a live form endpoint needs real spam protection wired in.
  //
  // The template always ships a working challenge, so a demo with an endpoint is never
  // unprotected and nobody has to stop and register Turnstile keys mid-build. That check
  // is client-side though, which is fine for filtering junk and not enough for a real
  // client's public endpoint, so this only bites on --live.
  // Cloudflare's documented test keys always pass, which is exactly what a demo wants and
  // exactly what a real client's site must never ship. They all start with the same
  // prefixes, so the check does not need to enumerate every one.
  const TEST_KEY = /^(1x0000|2x0000|3x0000)/;
  if (liveFlag && data.forms && TEST_KEY.test(data.forms.turnstileSiteKey || '')) {
    fail(
      `Gate 8 failed: forms.turnstileSiteKey is "${data.forms.turnstileSiteKey}", one of Cloudflare's always-passes test keys. It is the right key for a demo and useless on a live site. Replace it with the client's own site key from their Cloudflare dashboard.`
    );
  }
  if (liveFlag && data.forms && data.forms.endpoint && !data.forms.turnstileSiteKey) {
    fail(`Gate 8 failed: forms.endpoint is set but forms.turnstileSiteKey is empty on a --live build. The built-in challenge is client-side only, so add the Cloudflare Turnstile site key, and verify its token server-side, before a real client's endpoint goes public.`);
  }
...
function buildClient(slug, liveFlag) {
  ...
  // Render everything into memory first, so nothing touches disk until every page
  ...
  for (const file of templateFiles) {
    ...
    const outName = file.replace(/\.template\.([A-Za-z0-9]+)$/, '.$1');
    const html = render(tree, view, []);
    rendered[outName] = outName.endsWith('.html') ? minifyHtml(html) : html;
  }

  // Gate 12: demo-phase language must not survive a --live build.
  //
  // Every "design preview" string is wrapped in {{#if meta.noindex}}, but that is a
  // promise the templates make and this is the check that it was kept. A real client's
  // site whose <title> reads "Design preview (sample business)", or whose llms.txt tells
  // every AI crawler the business is not real, is the worst failure this repo can ship,
  // and it would be invisible to every other gate.
  if (liveFlag) {
    // Visible copy only. The .demo-ribbon CSS class ships either way, because the
    // stylesheet is shared and only the markup is conditional, so matching the class
    // name here would fail every live build for no reason.
    const DEMO_LANGUAGE = /design preview|sample business|not a real company|sample content|sample review/i;
    for (const [outName, html] of Object.entries(rendered)) {
      const hit = DEMO_LANGUAGE.exec(html);
      if (hit) {
        const at = Math.max(0, hit.index - 50);
        fail(
          `Gate 12 failed: ${outName} still contains demo-phase language on a --live build, near "…${html.slice(at, hit.index + 60).replace(/\s+/g, ' ')}…". ` +
            `A real client's site must not describe itself as a preview. Wrap it in {{#if meta.noindex}} in the template, or remove it.`
        );
      }
    }
  }
  ...
  // and trust-bar arrays), so an apostrophe in a business or gallery name, such as "O'Brien
